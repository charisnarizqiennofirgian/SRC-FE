      ...
      <div class="header-content-wrapper">
        <div class="header-left-section">
          ...
          <div class="header-text-content">
            <h1 class="page-title-sanding">Sampel — Sanding</h1>
      ...
      <div class="card-body-sanding">
        <form @submit.prevent="handleSubmit" @keydown.enter.prevent>
          ...
          <div class="form-actions-modern">
            ...
            <button type="submit" class="btn-action btn-submit-sanding" :disabled="isSubmitting">
              <span class="btn-icon">💾</span>
              <span class="btn-text">{{ isSubmitting ? 'Menyimpan...' : 'Simpan Sanding' }}</span>
            </button>
          </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    router.push({ name: 'SampelPacking' })
  } catch (error) {
    const message = error.response?.data?.message || 'Gagal menyimpan sanding'
...
.icon-badge-sanding { width: 72px; height: 72px; border-radius: 18px; background: rgba(255,255,255,0.25); backdrop-filter: blur(10px); display: flex; align-items: center; justify-content: center; }
.sanding-icon { font-size: 2.5rem; }
.page-title-sanding { font-size: 2rem; font-weight: 800; color: white; margin: 0 0 0.5rem; }
.page-subtitle-sanding { color: rgba(255,255,255,0.95); font-size: 1rem; margin: 0; font-weight: 500; }
...
.flow-label-sanding { color: rgba(255,255,255,0.9); font-size: 0.85rem; font-weight: 600; margin-top: 6px; text-align: center; }
.header-badge-section { display: flex; flex-direction: column; align-items: center; }

.content-card-sanding { background: white; border-radius: 20px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); border: 1px solid #f0f2f5; overflow: hidden; }
.card-body-sanding { padding: 2.5rem; }
...
.section-header { display: flex; align-items: center; gap: 1.25rem; margin-bottom: 1.75rem; padding: 1.25rem 1.5rem; background: linear-gradient(135deg, #f0f9ff, #e0f2fe); border-radius: 14px; border-left: 5px solid #0369a1; }
.section-icon-badge { width: 44px; height: 44px; border-radius: 12px; background: linear-gradient(135deg, #0369a1, #075985); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.form-input-modern:focus { outline: none; border-color: #0369a1; box-shadow: 0 0 0 4px rgba(3,105,161,0.15); }
...
.toggle-btn.active { border-color: #0369a1; background: #f0f9ff; color: #0369a1; }

.po-selected-info { display: flex; align-items: center; gap: 10px; padding: 12px 16px; background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 10px; margin-top: 1rem; }
...
.stock-hint { margin-left: 8px; font-size: 0.78rem; color: #0369a1; font-weight: 600; }
...
.btn-add-item { background: #f0f9ff; border-color: #0369a1; color: #075985; }
...
.btn-submit-sanding { background: linear-gradient(135deg, #0891b2, #0e7490); color: white; box-shadow: 0 4px 12px rgba(8,145,178,0.35); }
.btn-submit-sanding:hover:not(:disabled) { transform: translateY(-1px); }
.btn-submit-sanding:disabled { opacity: 0.6; cursor: not-allowed; }
...
.vue-select-item.vs--open :deep(.vs__dropdown-toggle) { border-color: #0369a1; box-shadow: 0 0 0 4px rgba(3,105,161,0.15); }
