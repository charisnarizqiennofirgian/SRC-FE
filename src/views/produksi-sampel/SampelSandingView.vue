  ...
  <DashboardLayout>
    <div class="page-header">
      <div style="display:flex;align-items:center;gap:1.5rem;">
        ...
        <div>
          <h1 class="page-title">Sanding — Produksi Sampel</h1>
          <p class="page-sub">Komponen masuk sanding → Gudang SANDING</p>
        </div>
      </div>
    </div>
    ...
    <div class="content-card">
      <div class="card-body">
        <form @submit.prevent="handleSubmit" @keydown.enter.prevent>
          ...
          <div class="form-section">
            <div class="section-hd"><div class="s-badge yellow"><span>📦</span></div><h3 class="s-title">Item yang Disanding *</h3></div>
            ...
            <template v-else>
              ...
              <button type="button" class="btn-add a-yellow" @click="form.items.push(newItem())">➕ Tambah Item</button>
            </template>
          </div>

          <div class="actions">
            <button type="button" class="btn-cancel" @click="router.back()">↩️ Batal</button>
            <button type="submit" class="btn-submit" :disabled="isSubmitting">💾 {{ isSubmitting?'Menyimpan...':'Simpan Sanding Sampel' }}</button>
          </div>
        </form>
      </div>
    </div>
...
const warehouseOpts = computed(() => warehouses.value.filter(w => ['MESIN','ASSEMBLING','PROTOTYPE'].includes(w.code)))
...
const handleSubmit = async () => {
  ...
  try {
    ...
    showSuccess('Sukses', 'Sanding sampel berhasil dicatat')
...
<style scoped>
.page-header { background:linear-gradient(135deg,#b45309,#92400e);padding:1.75rem 2rem;border-radius:16px;margin-bottom:1.75rem; }
...
.section-hd { display:flex;align-items:center;gap:.875rem;margin-bottom:1.25rem;padding:.875rem 1.125rem;background:#f9fafb;border-radius:10px;border-left:4px solid #b45309; }
.s-badge { width:36px;height:36px;border-radius:8px;background:linear-gradient(135deg,#b45309,#92400e);display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0; }
.s-badge.yellow { background:linear-gradient(135deg,#d97706,#b45309) !important; }
...
.fi:focus { outline:none;border-color:#b45309; }
.vs :deep(.vs__dropdown-toggle) { padding:.6rem .875rem;border:2px solid #e5e7eb;border-radius:8px;min-height:44px; }
.po-info { padding:.6rem 1rem;background:#fef3c7;border:1px solid #fde68a;border-radius:8px;margin-top:.25rem;font-size:.875rem; }
...
.stock-hint { font-size:.78rem;color:#b45309;font-weight:600;margin-top:3px; }
...
.a-yellow { border-color:#d97706;color:#b45309; }
...
.btn-submit { padding:.7rem 1.625rem;border-radius:8px;background:linear-gradient(135deg,#b45309,#92400e);color:white;border:none;font-weight:700;cursor:pointer; }
