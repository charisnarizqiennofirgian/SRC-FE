      ...
      <div class="header-content-wrapper">
        <div class="header-left-section">
          ...
          <div class="header-text-content">
            <h1 class="page-title-moulding">Moulding — Produksi Sampel</h1>
            <p class="page-subtitle-moulding">
              Catat pemakaian RST, hasil komponen, dan reject untuk setiap Production Order sampel.
            </p>
          </div>
        </div>
        ...
        <form @submit.prevent="handleSubmit" @keydown.enter.prevent>
              ...
              <div class="form-group-modern">
                ...
                <div class="input-wrapper-icon">
                  ...
                  <input
                    ...
                    placeholder="Catatan proses moulding sampel..."
                  />
                </div>
              </div>
            </div>
          ...
          <div class="form-section-modern">
            ...
            <div v-for="(group, gi) in form.groups" :key="group.local_id" class="group-card">
              ...
              <div class="line-block line-block--input">
                ...
                <div v-for="(inp, ii) in group.inputs" :key="inp.local_id" class="input-row">
          ...
          <div class="form-actions-modern">
            ...
            <button type="submit" class="btn-action btn-submit-modern" :disabled="isSubmitting">
              <span class="btn-icon">💾</span>
              <span class="btn-text">{{ isSubmitting ? 'Menyimpan...' : 'Simpan Moulding Sampel' }}</span>
            </button>
          </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    showSuccess('Sukses', 'Proses Moulding Sampel berhasil dicatat')
    ...
    const message =
      ...
      'Gagal menyimpan moulding sampel'
...
const tandaiSelesai = async () => {
  ...
  if (!confirm('Semua produk sudah moulding. Tandai PO selesai moulding?')) return

  isMarkingDone.value = true
  try {
    await apiClient.post(`/produksi/moulding/${form.ref_po_id}/selesai`)
    showSuccess('Selesai', 'PO berhasil ditandai selesai moulding')
    router.back()
  } catch (error) {
...
.btn-selesai-modern { background: linear-gradient(135deg, #0891b2, #0e7490); color: white; box-shadow: 0 4px 12px rgba(8,145,178,0.35); }
.btn-selesai-modern:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 6px 16px rgba(8,145,178,0.45); }
