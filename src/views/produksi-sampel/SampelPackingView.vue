      ...
      <div class="header-content-wrapper">
        <div class="header-left-section">
          ...
          <div class="header-text-content">
            <h1 class="page-title-packing">Sampel — Packing</h1>
...
const fetchInitialData = async () => {
  try {
    ...
    productionOrders.value = (poRes.data.data?.data || poRes.data.data || []).map(po => ({ ...po, label: po.po_number }))
...
.btn-submit-packing { background: linear-gradient(135deg, #0891b2, #0e7490); color: white; box-shadow: 0 4px 12px rgba(8,145,178,0.35); }
