...
const downloadTemplate = async () => {
  ...
  try {
    ...
    link.href = url
    link.setAttribute('download', 'Template_Supplier.xlsx')
...
const handleImportFile = async (e) => {
  ...
  try {
    ...
    showSuccess(
      'Import Berhasil!',
      `${res.data.imported} supplier diimport, ${res.data.skipped} dilewati.`
    )
...
const exportExcel = async () => {
  ...
  try {
    const res = await apiClient.get('/suppliers/export', { responseType: 'blob' })
    const url  = window.URL.createObjectURL(new Blob([res.data]))
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', `Data_Supplier_${new Date().toISOString().slice(0,10)}.xlsx`)
    document.body.appendChild(link)
    link.click()
    link.remove()
    window.URL.revokeObjectURL(url)
