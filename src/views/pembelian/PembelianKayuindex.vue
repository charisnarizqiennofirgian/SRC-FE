    ...
    <div class="page-header">
      <div class="header-content">
        <div class="header-left">
          ...
          <div>
            <h1 class="page-title">Daftar Pesanan Pembelian Kayu RST</h1>
            <p class="page-subtitle">
              Kelola purchase order khusus kayu RST (Round, Square, Timber)
            </p>
          </div>
        </div>
        <div class="header-actions">
          <router-link :to="{ name: 'BuatPembelianKayu' }" class="btn-header btn-primary">
            ...
            <tr v-for="pesanan in daftarPesanan" :key="pesanan.id" class="data-row">
              ...
              <td class="td-action">
                <div class="action-buttons">
                  ...
                  <button
                    v-if="pesanan.status === 'Diterima Sebagian'"
                    class="btn-action btn-tutup"
                    title="Tutup PO"
                    @click="tutupPO(pesanan)"
                  >
                    <span>🔒</span>
                  </button>
                  ...
                  <router-link
                    :to="{ name: 'CetakPesananPembelian', params: { id: pesanan.id }, query: { internal: '1' } }"
                    class="btn-action btn-print-internal"
                    title="Cetak Internal (dengan Peruntukan)"
                    target="_blank"
                  >
                    <span>📄</span>
                  </router-link>
...
const tutupPO = async (pesanan) => {
  if (!confirm(`Tutup PO ${pesanan.po_number}? PO tidak bisa menerima barang lagi setelah ditutup.`)) return
  try {
    const res = await apiClient.post(`/goods-receipts/${pesanan.id}/tutup`)
    toast.success(res.data.message)
    fetchDaftarPesanan()
  } catch (err) {
    toast.error(err.response?.data?.message || 'Gagal menutup PO')
  }
}
